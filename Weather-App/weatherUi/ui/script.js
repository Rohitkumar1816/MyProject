...
function getWeatherIcon(condition) {
    ...
    if (cond.includes('sunny') || cond.includes('clear')) return weatherIcons.sunny;
    if (cond.includes('partly') || cond.includes('partial')) return weatherIcons['partly-cloudy'];
    if (cond.includes('rain') || cond.includes('shower') || cond.includes('drizzle')) return weatherIcons.rainy;
    if (cond.includes('storm') || cond.includes('thunder')) return weatherIcons.stormy;
    if (cond.includes('cloud') || cond.includes('overcast')) return weatherIcons.cloudy;
...
function getCurrentTime() {
    const now = new Date();
    return now.toLocaleString('en-US', { weekday:'long', hour:'2-digit', minute:'2-digit', hour12:true });
}
...
function renderWeatherData(data) {
    ...
    const content = `
        ...
        <div class="weather-details">
            <div class="detail-item"><div class="label">Location</div><div class="value">${weatherResponse.city}, ${weatherResponse.country}</div></div>
            <div class="detail-item"><div class="label">Condition</div><div class="value">${weatherResponse.condition}</div></div>
            <div class="detail-item"><div class="label">Region</div><div class="value">${weatherResponse.region}</div></div>
        </div>
            ...
            <div class="forecast-container">
                ${dayTemp.map((day,index) => {
                    ...
                    return `<div class="forecast-day">
                        ...
                        <div class="temp-range"><span class="temp-max">${Math.round(day.maxTemp)}°</span> <span class="temp-min">${Math.round(day.minTemp)}°</span></div>
                    </div>`;
                }).join('')}
            </div>
        </div>
...
async function loadWeatherData(city, days) {
    try {
        ...
        const apiUrl = buildApiUrl(city, days);
        ...
        const weatherData = {
            weatherResponse: {
                city: data.weatherReasponse.City,
                region: data.weatherReasponse.Region,
                country: data.weatherReasponse.Country,
                condition: data.weatherReasponse.Condition,
                temperature: data.weatherReasponse.Temp
            },
            dayTemp: data.days.map(day => ({
                date: day.date,
                maxTemp: day.maxTem,
                minTemp: day.minTem
            }))
...
// Expose refresh to global scope
